...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  ...
  try {
    ...
    conn.sendFile(m.chat, r2es, 'error.mp4', `*_𝑺𝛩𝐾𝑈𝑁𝐴 | 𝑉𝐸𝑁𝛩𝑀_*`, m);
  } catch (err1) {
    try {
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                for (const {url, isVideo} of result.reverse()) await conn.sendFile(m.chat, url, `facebook.${!isVideo ? 'bin' : 'mp4'}`, '*_𝐕𝐄𝐍𝐎𝐌 | 𝐒𝐎𝐔𝐊𝐍𝐀_*', m);
              } catch (err6) {
                throw `*❗خطاء*`;
              }
            }
